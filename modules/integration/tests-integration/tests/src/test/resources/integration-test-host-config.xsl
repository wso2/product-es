<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="automation_mapping.xsd">
   ...
   <!-- <xsl:template match="xs:instance[@name='keyManager']/xs:hosts/xs:host/text()">localhost</xsl:template>
    ...
    <!--setting ports-->
    <xsl:template match="xs:instance/xs:ports/xs:port[@type='http']/text()">10663</xsl:template>
    <xsl:template match="xs:instance/xs:ports/xs:port[@type='https']/text()">10343</xsl:template>
    <!--<xsl:template match="xs:instance/xs:ports/xs:port[@type='nhttp']/text()">8780</xsl:template>
    <xsl:template match="xs:instance/xs:ports/xs:port[@type='nhttps']/text()">8743</xsl:template>-->
