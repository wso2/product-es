...
var tags, init, assets, asset, assetLinks, tagged, popularAssets, recentAssets, assetTypes, currentAsset,
    cache, cached, invalidate, comments, comment, assetsPaging, commentsPaging, rate, rating, assetCount, tenant,
    userAssets, registry, commentCount, search, addAsset, updateAsset, removeAsset, assetManager, addRxtConfigs;
...
(function() {
	...
	init = function(tenantId, options) {
    ...
    userAssets = function() {
		var items = [],
            ...
            path = user.userSpace() + '/userAssets',
	 ...
	 */
	tags = function(type) {
		var tag, tags, assetType, i, length, count, query, path,
            tagz = [],
            server = require('/modules/server.js'),
    ...
    initTenant = function(tenantId, tenantDomain) {
        request.tenantId = tenantId;
    };

    tenant = function() {
        return request.tenantId || session.get(STORE_TENANT);
    };
}());
