...
var rxt_converter = function () {
    ...
    mnger.registerHandler(new Handler({ tag: 'content', fn: function (context) {
        ...
        in
        ...
        )
            {
                ...
                objName.label = getLabel(field);//field['name'].@label;
    ...
    /**
     * The method returns the label of a given field by checking for a label attribute in the
     * name child element.If there is no child element then an empty string is returned.If there is
     * no label then the name is set as the label
     * @param e4xElement An E4X representing a field in the rxt
     * @returns The label of the field
     */
    var getLabel = function (e4xElement) {
        var value = '';
        if (e4xElement['name']) {
            value = e4xElement['name'].@label;
        }

        if (value == '') {
            value = e4xElement['name'];
        }
        return value;
    }
