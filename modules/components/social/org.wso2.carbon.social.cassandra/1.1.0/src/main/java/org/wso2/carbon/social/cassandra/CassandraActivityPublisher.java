...
public class CassandraActivityPublisher implements ActivityPublisher {
    ...
    private String getStreamId(DataPublisher publisher) {
        ...
        if (streamId == null && publisher != null) {
            try {
                streamId = publisher.findStreamId(Constants.STREAM_NAME, Constants.STREAM_VERSION);
                /*if (streamId == null) {
                    try {
                ...
                LOG.error("Can't find " + Constants.STREAM_NAME + ":" +
                        Constants.STREAM_VERSION + " for storing social Activities", e);
            }
        }
        return streamId;
    }
