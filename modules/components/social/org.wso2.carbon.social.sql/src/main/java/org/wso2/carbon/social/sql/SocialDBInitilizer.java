...
/**
 * This class executes the sql script in dbscripts/social folder to initialize the database
 * tables during startup.
 */
public class SocialDBInitilizer {

	private static Log log = LogFactory.getLog(SocialDBInitilizer.class);
	private static final String DB_CHECK_SQL = "select 1 from " + Constants.SOCIAL_COMMENTS_TABLE_NAME;
	private DataSource dataSource;
	private String delimiter = ";";
	Statement statement;

	SocialDBInitilizer(DataSource dataSource) {
		this.dataSource = dataSource;
	}

	void createSocialDatabase() throws Exception {
	...
	protected String getDbScriptLocation(String databaseType) {
		...
		return carbonHome +
				"/dbscripts/social/" + databaseType + "/resource.sql";
	}
