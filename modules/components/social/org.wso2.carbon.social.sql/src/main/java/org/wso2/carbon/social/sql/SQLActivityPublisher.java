...
public class SQLActivityPublisher extends ActivityPublisher {
    ...
    protected String publish(String id, NativeObject activity) {
        ...
        if (connection != null) {
            ...
            try {
                ...
                String contextId = JSONUtil.getNullableProperty(activity, CONTEXT_JSON_PROP, ID_JSON_PROP);
                String userId = JSONUtil.getNullableProperty(activity, ACTOR_JSON_PROP, ID_JSON_PROP);
                String rating = JSONUtil.getNullableProperty(activity, OBJECT_JSON_PROP, RATING_JSON_PROP);
                if (contextId == null) {
                    contextId = JSONUtil.getProperty(activity, TARGET_JSON_PROP, ID_JSON_PROP);
                }
                if (userId == null) {
                    userId = JSONUtil.getProperty(activity, ACTOR_JSON_PROP, ID_JSON_PROP);
                }
                if (rating == null) {
                    rating = JSONUtil.getProperty(activity, OBJECT_JSON_PROP, RATING_JSON_PROP);
                }
    ...
    public boolean vote(String commentId, String actorId, int vote) {
        ...
        if (connection != null) {
            ...
            try {
                statement = connection.prepareStatement(SELECT_VOTE_SQL);
                statement.setString(1, commentId);
                resultSet = statement.executeQuery();
                resultSet.next();

                JsonObject votes = (JsonObject) parser.parse(resultSet.getString(VOTE_COLUMN));
                ...
                update = connection.prepareStatement(UPDATE_VOTE_SQL);
