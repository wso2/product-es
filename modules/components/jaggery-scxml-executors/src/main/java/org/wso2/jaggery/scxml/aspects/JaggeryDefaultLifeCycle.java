...
/*
NOT USED, keep walking Bub!

Fellow Lost Soul,
Keep walking, there is nothing to see here

P.S: If you really must don't poke around inside too much.

Source from: package org.wso2.carbon.governance.filters.extensions.aspects;
Yours sincerely,
The Guy who copied and pasted this!
 */
public class JaggeryDefaultLifeCycle extends Aspect {

    private static final Log log = LogFactory.getLog(DefaultLifeCycle.class);

    private String lifecycleProperty = "filters.LC.name";
    private String stateProperty = "filters.lifecycle.SoftwareProjectLifecycle.state";
    private String stateVoteProperty = "filters.LC.currentVotes";
    ...
    public JaggeryDefaultLifeCycle(OMElement config) throws RegistryException {
...
//        Checking whether the lifecycles variables are properly initialized. If not, we initialize them again.
	...
	statCollection.setAspectName(aspectName);
        ...
        /*****************************************************************
         *  CHANGE: This action is not required as we do not
         *          change the path
         *          If this is present it will throw an exception as
         *          the permissions might be changed by a custom
         *          executor.
         *  Date Modified: 30/8/2013
         *  Made by : Jaggery Team
         *****************************************************************/
        ...
        //        adding the logs to the filters
        if (isAuditEnabled) {
            StatWriter.writeHistory(statCollection);
        }
    }
    ...
    private void executeJS(String script) throws Exception{
        ...
        try {
            ...
            String msg = "Unable to defining filters host objects.";
            throw new Exception(msg,e);
        } catch (InstantiationException e) {
            String msg = "Unable to instantiate the given filters host object.";
            throw new Exception(msg,e);
        } catch (InvocationTargetException e) {
            String msg = "An exception occurred while creating filters host objects.";
            throw new Exception(msg,e);
        } catch (AxisFault e) {
